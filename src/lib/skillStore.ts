import { Skill, SkillStep, Agent } from '@/types';
...
export function getSkills(): Skill[] {
  ...
  if (!raw) {
    // Seed with built-in skills on first load
...
function getBuiltInSkills(): Skill[] {
  return [
    ...
    },
    {
      ...
      inputSchema: [
        { name: 'focus', type: 'string', description: 'What aspect to focus on (bugs, performance, style)', required: false },
      ],
      steps: [
    ...
    },
    {
      ...
      requiredPermissions: [],
      inputSchema: [
        { name: 'subject', type: 'string', description: 'The subject to analyze', required: true },
      ],
      steps: [
...
export function buildSkillsPromptBlock(agent: Agent, latestUserMessage?: string): string {
  ...
  // Check which skills are triggered by the latest message
  ...
  for (const skill of skills) {
    ...
    if (missingPerms.length > 0) continue; // skip skills agent can't execute
